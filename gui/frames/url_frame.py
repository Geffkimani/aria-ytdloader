...
class URLFrame(ttk.Frame):
    def __init__(self, master, app_logic):
        ...
        # self.cancel_button is packed/unpacked in set_state

        # Pause/Resume controls (POSIX best-effort)
        self.pause_button = ttk.Button(button_frame, text="Pause", command=self.app.pause_all, width=8, bootstyle=SECONDARY)
        self.resume_button = ttk.Button(button_frame, text="Resume", command=self.app.resume_all, width=8, bootstyle=INFO)

    def on_drop(self, event):
        # Strip curly braces that might be added by some drag-and-drop sources
