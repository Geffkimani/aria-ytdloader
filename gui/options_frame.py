...
class OptionsFrame(ttk.Frame):
    def __init__(self, master, app_logic):
        ...
        self.quality_combobox = ttk.Combobox(left_frame, textvariable=self.app.quality_var, values=['1080p','720p','480p','360p'], state="readonly", width=10)
        self.quality_combobox.pack(side=LEFT, padx=(0,5))
        ttk.Label(left_frame, text="Quality:").pack(side=LEFT)
        self.audio_only_checkbox = ttk.Checkbutton(left_frame, text="Audio Only", variable=self.app.audio_only_var, bootstyle="round-toggle", command=self.app.update_option_states)
        self.audio_only_checkbox.pack(side=LEFT, padx=15)
        self.embed_thumbnail_checkbox = ttk.Checkbutton(left_frame, text="Embed Thumbnail", variable=self.app.embed_thumbnail_var, bootstyle="round-toggle")
        self.embed_thumbnail_checkbox.pack(side=LEFT, padx=5)
        right_frame = ttk.Frame(self)
        right_frame.pack(side=RIGHT, fill=X, padx=5)
        self.select_folder_button = ttk.Button(right_frame, text="Save Location", command=self.app.select_folder, width=15, bootstyle=OUTLINE)
        self.select_folder_button.pack(side=LEFT, padx=5)
        self.settings_button = ttk.Button(right_frame, text="Settings", command=self.app.open_settings, width=10, bootstyle=INFO)
